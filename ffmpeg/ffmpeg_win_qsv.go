//go:build windows && qsv
// +build windows,qsv

package ffmpeg

/*
#cgo windows CFLAGS: -I${SRCDIR}/../libs/windows/ffmpeg_qsv/include/ -I${SRCDIR}/../libs/windows/mfx/
#cgo windows LDFLAGS: -L${SRCDIR}/../libs/windows/ffmpeg_qsv/bin/ -lavformat -lavutil -lavcodec -lswscale
#include <windows.h>
#include "ffmpeg.h"
*/
import "C"
...
func newFFCtxByCodec(codec *C.AVCodec) (ff *ffctx, mode DecodeMode, err error) {
	...
	ret := C.qsv_codec_init(&ff.ff.hw_device_ctx)
	if ret >= C.int(0) {
		codec = C.qsc_codec_finder()
	}
	ff.ff.codec = codec
	...
	if ret >= C.int(0) {
		C.qsv_codec_setup(ff.ff.codecCtx, ff.ff.hw_device_ctx)
		mode = DecodeModeQSV
		log.Println("load hardware decode success mode:", mode)
	}

...
func (self *VideoDecoder) Setup() (err error) {
	...
	log.Infof("load hardware decode mode:%v", self.Mode)
...
func (self *VideoDecoder) Decode(pkt []byte) (img *VideoFrame, err error) {
	...
	var frame *C.AVFrame
	var cerr C.int
	log.Debugf("self.Mode :%v", self.Mode)
	switch self.Mode {
	case DecodeModeCuda:
		cerr = C.wrap_avcodec_decode_video_with_hw(ff.codecCtx, &frame, unsafe.Pointer(&pkt[0]), C.int(len(pkt)), &cgotimg)
	case DecodeModeQSV:
		cerr = C.wrap_avcodec_decode_video_with_hw(ff.codecCtx, &frame, unsafe.Pointer(&pkt[0]), C.int(len(pkt)), &cgotimg)
	default:
	...
	if cerr < C.int(0) {
		err = fmt.Errorf("ffmpeg: wrap_avcodec_decode_video failed: %d,%v,mode:%v", int(cerr), cgotimg, self.Mode)
		return
	}
	log.Debug("XXXXXXXXXXXXXXXXXXXXXXwrap_avcodec_decode_video", time.Since(start))
	if cgotimg != C.int(0) {

		if frame.format == C.AV_PIX_FMT_NV12 {
			// img = nv12Image(frame)
			img = nv12GoByte(frame)
		} else if frame.format == C.AV_PIX_FMT_YUV420P || frame.format == C.AV_PIX_FMT_YUVJ420P {
			img = yuvGoByte(frame)
		}

...
// IsTSVideoType check if it is a video encoding format supported by TS stream
func IsTSVideoType(streamType uint8) (supported bool) {

	if streamType == ElementaryStreamTypeH264 {
		supported = true
	}
...
// IsTSAudioType check if it is a audio encoding format supported by TS stream
func IsTSAudioType(streamType uint8) (supported bool) {

	if streamType == ElementaryStreamTypeAdtsAAC {
		supported = true
	}
...
func NewTSWriter(pid uint16) *TSWriter {
	w := &TSWriter{}
	w.tshdr = make([]byte, TSPacketSize)
	w.tshdr[0] = 0x47
...
func (self *TSWriter) WritePackets(w io.Writer, datav [][]byte, pcr time.Duration, sync bool, paddata bool) (err error) {
	...
	for writepos < datavlen {
		...
		padtail := 0
		end := writepos + TSPacketSize - hdrlen
		if end > datavlen {
			if paddata {
		...
		if padtail > 0 {
			if _, err = w.Write(self.tshdr[TSPacketSize-padtail : TSPacketSize]); err != nil {
				return
			}
		}
...
func ParseTSHeader(tshdr []byte) (pid uint16, start bool, iskeyframe bool, hdrlen int, hasPayload bool, err error) {
	...
	hdrlen += 4
	hasOptional := (tshdr[3] & 0x20) != 0
	if hasOptional {
		hdrlen += int(tshdr[4]) + 1
		iskeyframe = tshdr[5]&0x40 != 0
	}
	hasPayload = (tshdr[3] & 0x10) != 0
	return
}

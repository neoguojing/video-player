...
func NewMetadataByStreams(streams []av.CodecData) (metadata flvio.AMFMap, err error) {
	...
	for _, _stream := range streams {
		...
		case typ.IsAudio():
			...
			case av.PCM_ALAW:
				metadata["audiocodecid"] = flvio.SOUND_ALAW

			case av.PCM_MULAW:
				metadata["audiocodecid"] = flvio.SOUND_MULAW
...
func PacketToTag(pkt av.Packet, stream av.CodecData) (tag flvio.Tag, timestamp int32, ok bool, err error) {
	switch stream.Type() {
	case av.H264:
		tag = flvio.Tag{
			Type:            flvio.TAG_VIDEO,
		...
		// FLV cannot include SEI/PPS/SPS
		/*
			naltype := tag.Data[4] & 0x1f
	...
	case av.MP3:
		// XXX handle time
		return
		var mp3 mp3parser.CodecData
		mp3, err = mp3parser.NewCodecDataFromMP3AudioConfigBytes(pkt.Data)
		if err != nil {
			err = nil
			return
		}
		tag = flvio.Tag{
			Type:        flvio.TAG_AUDIO,
			SoundFormat: flvio.SOUND_MP3,
			SoundSize:   flvio.SOUND_16BIT,
			Data:        pkt.Data,
		}
		switch mp3.SampleRate() {
		case 11025:
			tag.SoundRate = flvio.SOUND_11Khz
		case 22050:
			tag.SoundRate = flvio.SOUND_22Khz
		case 44100:
			tag.SoundRate = flvio.SOUND_44Khz
		case 48000:
			tag.SoundRate = flvio.SOUND_44Khz
		default:
			err = fmt.Errorf("flv: unsupported sample rate=%v", mp3.SampleRate())
			return
		}
		switch mp3.ChannelLayout().Count() {
		case 1:
			tag.SoundType = flvio.SOUND_MONO
		case 2:
			tag.SoundType = flvio.SOUND_STEREO
...
func (self *Muxer) WritePacket(pkt av.Packet) (err error) {
	...
	tag, timestamp, ok, err := PacketToTag(pkt, stream)
	if err != nil {
		return
	}
	if !ok {
